...
const FEATURES = [
    ...
    { id: 'fishWeightPred', name: '鱼体力预估体重' },
    ...
    { id: 'mainLineCutCount', name: '主线切线计算' },
...
function initFloatingPanel() {
    ...
    // 样式注入
    // ============================================================
    const styleId = 'lazyfisher-floating-panel-style';
    ...
    header.innerHTML = `
        <span id="lazyfisher-panel-title">⚙ 功能开关</span>
        <span id="lazyfisher-panel-count"></span>
        <button id="lazyfisher-panel-close">✕</button>
    `;
    ...
    function buildToggleRows() {
        ...
        FEATURES.forEach(f => {
            ...
            row.innerHTML = `
                <span>${f.name}</span>
                <label class="lazyfisher-toggle-switch">
                    <input type="checkbox" ${isEnabled(f.id) ? 'checked' : ''} data-feature="${f.id}">
                    <span class="lazyfisher-toggle-slider"></span>
                </label>
            `;
    ...
    function updateCount() {
        ...
        const onCount = FEATURES.filter(f => isEnabled(f.id)).length;
        document.getElementById('lazyfisher-panel-count').textContent = `${onCount}/${total} ON`;
    }
    updateCount();
    ...
    function openPanel() {
        ...
        const panelW = 300;
        const panelH = Math.min(window.innerHeight * 0.7, 500);
    ...
    document.getElementById('lazyfisher-panel-close').addEventListener('click', function(e) {
        e.stopPropagation();
        closePanel();
    });
...
// 功能：鱼体力预估体重
function initFishWeightPred() {
    (function() {
        'use strict';

        // ---- 存储键名（统一使用无版本号） ----
        const GLOBAL_DATA_KEY = 'fish_weight_global_data';
        const SPECIES_DATA_KEY = 'fish_weight_species_data';
        const REGIONAL_DATA_KEY = 'fish_weight_regional_data';
        const COEFF_KEY = 'fish_weight_coeff';

        // ---- 默认参数 ----
        const DEFAULT_SLOPE = 0.5;
        const DEFAULT_INTERCEPT = 0.0;
        const MIN_SAMPLES = 10;
        const MAX_SAMPLES_PER_SPECIES = 100;
        const MAX_GLOBAL_SAMPLES = 10000;
        const MAX_HISTORY_LEARN = 300;

        // ---- 运行时状态 ----
        const STATE = { IDLE: 0, FIGHT: 1, REPLAY: 2 };
        let currentState = STATE.IDLE;
        let maxStaminaThisFight = 0;
        let globalData = [];
        let speciesData = {};
        let regionalData = {};
        let speciesCoefCache = {};
        let globalSlope = DEFAULT_SLOPE;
        let globalIntercept = DEFAULT_INTERCEPT;
        let useGlobalLearned = false;
        let dataDirty = false;
        ...
        // ========== 存储读写 ==========
        function loadData() {
            try { globalData = GM_getValue(GLOBAL_DATA_KEY, []).slice(-MAX_GLOBAL_SAMPLES); } catch (e) { globalData = []; }
            try { speciesData = GM_getValue(SPECIES_DATA_KEY, {}); } catch (e) { speciesData = {}; }
            try { regionalData = GM_getValue(REGIONAL_DATA_KEY, {}); } catch (e) { regionalData = {}; }
        }

        function saveData() {
            GM_setValue(GLOBAL_DATA_KEY, globalData);
            GM_setValue(SPECIES_DATA_KEY, speciesData);
            GM_setValue(REGIONAL_DATA_KEY, regionalData);
            dataDirty = false;
        }
        ...
        function loadCoeff() {
            try {
                const raw = GM_getValue(COEFF_KEY);
                if (raw && typeof raw.slope === 'number') {
                    globalSlope = raw.slope;
                    globalIntercept = raw.intercept;
                    useGlobalLearned = true;
                }
            } catch (e) {}
        }

        function saveCoeff() {
            GM_setValue(COEFF_KEY, { slope: globalSlope, intercept: globalIntercept });
        }

        // ========== 回归计算 ==========
        function calcRegression(samples) {
            if (!samples || samples.length < MIN_SAMPLES) return null;
            const n = samples.length;
            let sx = 0, sy = 0, sxy = 0, sxx = 0;
            samples.forEach(d => { sx += d.stamina; sy += d.weight; sxy += d.stamina * d.weight; sxx += d.stamina * d.stamina; });
            const denom = n * sxx - sx * sx;
            if (Math.abs(denom) < 0.0001) return null;
            const slope = (n * sxy - sx * sy) / denom;
            const intercept = (sy - slope * sx) / n;
            if (slope < 0.1 || slope > 5.0) return null;
            return { slope, intercept };
        }

        function recalcGlobalRegression() {
            const result = calcRegression(globalData);
            if (result) { globalSlope = result.slope; globalIntercept = result.intercept; useGlobalLearned = true; }
            else { globalSlope = DEFAULT_SLOPE; globalIntercept = DEFAULT_INTERCEPT; useGlobalLearned = false; }
            saveCoeff();
        }

        function updateSpeciesCoefCache(fishName) {
            if (speciesData[fishName] && speciesData[fishName].length >= MIN_SAMPLES) {
                speciesCoefCache[fishName] = calcRegression(speciesData[fishName]);
            } else {
                speciesCoefCache[fishName] = null;
            }
        }

        function rebuildAllCoefCache() {
            speciesCoefCache = {};
            Object.keys(speciesData).forEach(name => updateSpeciesCoefCache(name));
        }

        // ========== 添加样本 ==========
        function addSample(stamina, weight, fishName, extraData = {}) {
            if (stamina <= 0 || weight <= 0) return;
            const cleanName = cleanFishName(fishName);
            if (!cleanName) return;
            ...
            const sample = {
                ...
                environment: extraData.environment,
                waterFlow: extraData.waterFlow,
                humidity: extraData.humidity
            };

            // 全局数据
            globalData.push({ stamina, weight });
            if (globalData.length > MAX_GLOBAL_SAMPLES) globalData = globalData.slice(-MAX_GLOBAL_SAMPLES);
            recalcGlobalRegression();

            // 鱼种数据
            if (!speciesData[cleanName]) speciesData[cleanName] = [];
            speciesData[cleanName].push(sample);
            if (speciesData[cleanName].length > MAX_SAMPLES_PER_SPECIES) {
                speciesData[cleanName] = speciesData[cleanName].slice(-MAX_SAMPLES_PER_SPECIES);
            }
            updateSpeciesCoefCache(cleanName);

            // 区域数据
            const loc = extraData.location;
            if (loc) {
                if (!regionalData[loc]) regionalData[loc] = {};
                if (!regionalData[loc][cleanName]) regionalData[loc][cleanName] = [];
                regionalData[loc][cleanName].push(sample);
                if (regionalData[loc][cleanName].length > MAX_SAMPLES_PER_SPECIES) {
                    regionalData[loc][cleanName] = regionalData[loc][cleanName].slice(-MAX_SAMPLES_PER_SPECIES);
                }
            }

            dataDirty = true;
            clearTimeout(captureTimeout);
            captureTimeout = setTimeout(flushData, 3000);
        }

        function cleanFishName(name) {
            if (!name) return null;
            let cleaned = name.trim().replace(/^(上鱼|回放)[：:]\s*/, '');
            return cleaned.length >= 2 ? cleaned : null;
        }

        // ========== 环境提取 ==========
        function extractEnvironmentAndRegion() {
            const card = document.querySelector('.fishing-compact-card');
            if (!card) return null;
            const items = card.querySelectorAll('.fishing-overview-item');
            let location = null;
            for (const item of items) {
                const label = item.querySelector('.text-xs.text-muted');
                if (label && label.textContent.includes('当前区域')) {
                    const strong = item.querySelector('strong');
                    location = strong ? strong.textContent.trim() : null;
                    break;
                }
            }
            if (location) currentLocation = location;

            const parseVal = (el) => el ? parseFloat(el.textContent.trim()) : null;
            const waterFlowEl = document.querySelector('#lz-ev span[title="水流"]');
            const humidityEl = document.querySelector('#lz-ev span[title="湿度"]');
            const windEl = document.querySelector('.weather-icon-row span[title^="风"]');
            const cloudEl = document.querySelector('.weather-icon-row span[title^="云"]');
            const rainEl = document.querySelector('.weather-icon-row span[title^="雨"]');

            return {
                location,
                waterFlow: parseVal(waterFlowEl),
                humidity: parseVal(humidityEl),
                environment: { wind: parseVal(windEl), cloud: parseVal(cloudEl), rain: parseVal(rainEl) }
            };
        }

        // ========== 体力提取 ==========
        function findStaminaMax(container = document) {
            const metrics = container.querySelectorAll('.fishing-metric-card');
            for (const card of metrics) {
                const label = card.querySelector('.fishing-metric-label');
                if (label && label.textContent.includes('鱼体力')) {
                    const valueEl = card.querySelector('.fishing-metric-value');
                    if (!valueEl) continue;
                    const text = valueEl.textContent.trim();
                    const parts = text.split('/');
                    const maxVal = parseFloat(parts.length === 2 ? parts[1].trim() : parts[0].trim());
        ...
        // ========== 弹窗提取 ==========
        function extractFishNameFromModal() {
            const modal = document.querySelector('.modal-body');
            if (!modal) return null;
            const strong = modal.querySelector('.card.mb-sm strong');
            return strong ? cleanFishName(strong.textContent) : null;
        }

        function extractWeightFromModal() {
            const modal = document.querySelector('.modal-body');
            if (!modal) return null;
            const detail = modal.querySelector('.card.mb-sm .text-sm.text-muted');
            if (!detail) return null;
            const match = detail.textContent.match(/([\d,.]+)\s*kg/);
            return match ? parseFloat(match[1].replace(/,/g, '')) : null;
        }

        function extractStaminaMaxFromModal() {
            const modal = document.querySelector('.modal-body');
            return modal ? findStaminaMax(modal) : null;
        }

        // ========== 鱼名推测 ==========
        function calcMatchScore(staminaMax, samples) {
            const staminaValues = samples.map(s => s.stamina);
            const minS = Math.min(...staminaValues);
            const maxS = Math.max(...staminaValues);
            const avgS = staminaValues.reduce((a, b) => a + b, 0) / staminaValues.length;
            if (staminaMax >= minS && staminaMax <= maxS) {
                return Math.abs(staminaMax - avgS) / (maxS - minS || 1);
            }
            if (staminaMax >= minS * 0.85 && staminaMax <= maxS * 1.15) {
                return Math.abs(staminaMax - avgS) / (maxS - minS || 1) + 0.5;
            }
            return null;
        }

        function predictFishNameByStamina(staminaMax) {
            if (!staminaMax || Object.keys(speciesData).length === 0) return null;
            let bestMatch = null, bestScore = Infinity;

            // 优先区域匹配
            const loc = currentLocation || extractEnvironmentAndRegion()?.location;
            if (loc && regionalData[loc]) {
                for (const [fishName, samples] of Object.entries(regionalData[loc])) {
                    if (samples.length < MIN_SAMPLES) continue;
                    const score = calcMatchScore(staminaMax, samples);
                    if (score !== null && score < bestScore) { bestScore = score; bestMatch = fishName; }
                }
                if (bestMatch) return bestMatch;
            }

            // 全局匹配
            for (const [fishName, samples] of Object.entries(speciesData)) {
                if (samples.length < MIN_SAMPLES) continue;
                const score = calcMatchScore(staminaMax, samples);
                if (score !== null && score < bestScore) { bestScore = score; bestMatch = fishName; }
            }
        ...
        function getActiveFishName() {
            const modalName = extractFishNameFromModal();
            if (modalName) { currentFishName = modalName; return modalName; }
            if (currentFishName) return currentFishName;
            const staminaMax = findStaminaMax();
            if (staminaMax) return predictFishNameByStamina(staminaMax) || null;
            return null;
        }
        ...
        function isReplayMode() {
            const modal = document.querySelector('.modal-body');
            return modal && modal.querySelector('.catch-rating-bar');
        }

        function isFightMode() {
            return !!document.querySelector('.fishing-metric-grid');
        }

        function updateState() {
            if (isReplayMode()) currentState = STATE.REPLAY;
            else if (isFightMode()) currentState = STATE.FIGHT;
            else currentState = STATE.IDLE;
        }

        // ========== 回放学习 ==========
        function checkReplayAndLearn() {
            if (currentState !== STATE.REPLAY) return;
            const fishName = extractFishNameFromModal();
            const weight = extractWeightFromModal();
            const staminaMax = extractStaminaMaxFromModal();
            if (!fishName || !weight || !staminaMax) return;

            const sig = fishName + '_' + staminaMax + '_' + weight;
            const now = Date.now();
            if (sig === lastReplaySignature && now - lastReplayTime < 10000) return;
            lastReplaySignature = sig;
            lastReplayTime = now;
            currentFishName = fishName;

            const envData = extractEnvironmentAndRegion() || {};
            addSample(staminaMax, weight, fishName, envData);
            updateDisplay();
        }

        // ========== 日志学习 ==========
        function checkFishingLogAndLearn() {
            const logList = document.querySelector('.fishing-log-list');
            if (!logList) return;
            const successLogs = logList.querySelectorAll('.fishing-log-item--success');
            if (successLogs.length === 0) return;
            const latestLog = successLogs[successLogs.length - 1];
            const messageEl = latestLog.querySelector('.fishing-log-message');
            if (!messageEl) return;

            const text = messageEl.textContent.trim();
            const match = text.match(/成功起鱼[：:]\s*(.+?)\s*·\s*([\d,.]+)\s*(kg|g)\s*·\s*\S+/);
            if (!match) return;

            const fishName = cleanFishName(match[1]);
            let weight = parseFloat(match[2].replace(/,/g, ''));
            if (match[3] === 'g') weight /= 1000;
            if (!fishName || weight <= 0) return;

            const logSig = fishName + '_' + weight + '_' + latestLog.querySelector('.fishing-log-time')?.textContent;
            if (logSig === lastReplaySignature) return;
            lastReplaySignature = logSig;
        ...
        // ========== 批量学习 ==========
        function batchLearnCatchHistory() {
            const cards = document.querySelectorAll('.card-list .message-card--catch');
            let learned = 0, skipped = 0;
            cards.forEach((card, index) => {
                if (index >= MAX_HISTORY_LEARN) return;
                const titleEl = card.querySelector('.message-title');
                const detailEl = card.querySelector('.text-sm.text-muted');
                if (!titleEl || !detailEl) { skipped++; return; }

                const fishName = cleanFishName(titleEl.textContent);
                const detailText = detailEl.textContent;
                const weightMatch = detailText.match(/([\d,.]+)\s*(kg|g)/);
                const lengthMatch = detailText.match(/\d+\.?\d*\s*(kg|g)\s*\/\s*([\d,.]+)\s*cm/);
                if (!fishName || !weightMatch) { skipped++; return; }

                let weight = parseFloat(weightMatch[1].replace(/,/g, ''));
                if (weightMatch[2] === 'g') weight /= 1000;
                let length = lengthMatch ? parseFloat(lengthMatch[2].replace(/,/g, '')) : null;
                const staminaMax = findStaminaMax();
                const envData = extractEnvironmentAndRegion() || {};

                if (staminaMax > 0 && weight > 0) {
                    addSample(staminaMax, weight, fishName, { length: length, location: envData.location, environment: envData.environment, waterFlow: envData.waterFlow, humidity: envData.humidity });
                    learned++;
                } else { skipped++; }
            });
            flushData();
            alert('批量学习完成！\n处理: ' + Math.min(cards.length, MAX_HISTORY_LEARN) + ' 条\n成功: ' + learned + ' 条\n跳过: ' + skipped + ' 条');
        }

        // ========== 预测 ==========
        function predictWeight(staminaMax, fishName) {
            if (staminaMax === null) return { weight: null, source: '无数据', fishDisplay: '未知' };
            let weight, source, useFishName = fishName || '未知';

            if (fishName && speciesCoefCache[fishName]) {
                const coef = speciesCoefCache[fishName];
                weight = Math.max(0, coef.slope * staminaMax + coef.intercept);
                source = '鱼种: ' + fishName;
            } else {
                weight = Math.max(0, globalSlope * staminaMax + globalIntercept);
                source = useGlobalLearned ? '通用(已优化)' : '通用(默认)';
                useFishName = fishName ? fishName + '(推测)' : '未知';
            }
            return { weight: weight.toFixed(1), source, fishDisplay: useFishName };
        }

        // ========== UI ==========
        function createMetricCard(labelText, valueText, iconSvg, extraClass) {
            const card = document.createElement('div');
            card.className = 'fishing-metric-card ' + (extraClass || '');
            card.innerHTML = '\n                <div class="fishing-metric-header">\n                    <span class="fishing-metric-label">' + iconSvg + escapeHtml(labelText) + '</span>\n                    <span class="fishing-metric-value" style="font-weight:bold;">' + escapeHtml(valueText) + '</span>\n                </div>\n                <div class="progress-bar fishing-metric-progress">\n                    <div class="progress-bar-fill progress-bar-fill--primary" style="width:0%"></div>\n                </div>';
            return card;
        }

        function getFishIconSvg() {
            return '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6.5 12c.94-3.46 4.94-6 8.5-6 3.56 0 6.06 2.54 7 6-.94 3.47-3.44 6-7 6s-7.56-2.53-8.5-6Z"/><path d="M18 12v.5"/><path d="M16 17.93A9.77 9.77 0 0 1 12 18"/><path d="M12 18a9.77 9.77 0 0 1-4-.07"/><path d="M6 12.5V12"/></svg>';
        }

        function injectWeightDisplay(weightText, sourceDesc, fishName) {
            const metrics = document.querySelectorAll('.fishing-metric-card');
            let targetCard = null;
            for (const card of metrics) {
                if (card.querySelector('.fishing-metric-label')?.textContent.includes('鱼体力')) { targetCard = card; break; }
            }
            if (!targetCard) return;

            const currentSig = fishName + '_' + weightText;
            if (currentSig === lastInjected) return;
            lastInjected = currentSig;

            const oldName = document.querySelector('.fish-weight-name-card');
            const oldPred = document.querySelector('.fish-weight-pred-card');
            if (oldName) oldName.remove();
            if (oldPred) oldPred.remove();

            const icon = getFishIconSvg();
            const nameCard = createMetricCard('鱼名', fishName, icon, 'fish-weight-name-card');
            const predCard = createMetricCard('预估(' + sourceDesc + ')', weightText + ' kg', icon, 'fish-weight-pred-card');
            targetCard.parentNode.insertBefore(predCard, targetCard.nextSibling);
            targetCard.parentNode.insertBefore(nameCard, predCard);
        }
        ...
        function updateDisplay() {
            const container = currentState === STATE.REPLAY ? document.querySelector('.modal-body') : document;
            const staminaMax = findStaminaMax(container);
            if (staminaMax === null) return;
            const fishName = getActiveFishName();
            if (!fishName) return;
            const result = predictWeight(staminaMax, fishName);
            if (result.weight) injectWeightDisplay(result.weight, result.source, result.fishDisplay);
        }

        // ========== 双向互通查询 ==========
        function buildFishToRegionMap() {
            const map = {};
            Object.entries(regionalData).forEach(([region, fishes]) => {
                Object.keys(fishes).forEach(fishName => {
                    if (!map[fishName]) map[fishName] = [];
                    map[fishName].push(region);
                });
            });
            return map;
        }
        ...
        function showQueryDialog() {
            const fishToRegion = buildFishToRegionMap();
            const allFish = Object.keys(speciesData).sort();
            const allRegions = Object.keys(regionalData).sort();

            let msg = '====== 双向互通查询 ======\n\n';
            msg += '通用样本: ' + globalData.length + ' | 斜率: ' + globalSlope.toFixed(4) + '\n';
            msg += '鱼种总数: ' + allFish.length + ' | 区域总数: ' + allRegions.length + '\n\n';

            msg += '--- 按鱼种查找区域 ---\n';
            allFish.forEach(fishName => {
                const samples = speciesData[fishName];
                const regions = fishToRegion[fishName] || [];
                const minS = Math.min(...samples.map(s => s.stamina));
                const maxS = Math.max(...samples.map(s => s.stamina));
                const avgW = (samples.reduce((a, s) => a + s.weight, 0) / samples.length).toFixed(2);
                msg += '\n[鱼种] ' + fishName + ' (' + samples.length + '条, 体力' + minS + '-' + maxS + ', 均重' + avgW + 'kg)\n';
                msg += '  出现区域: ' + (regions.length > 0 ? regions.map(r => '[区域] ' + r).join(', ') : '无区域数据') + '\n';
            });

            msg += '\n\n--- 按区域查找鱼种 ---\n';
            allRegions.forEach(region => {
                const fishes = Object.entries(regionalData[region]);
                const totalSamples = fishes.reduce((sum, [, s]) => sum + s.length, 0);
                msg += '\n[区域] ' + region + ' (' + Object.keys(regionalData[region]).length + '种鱼, ' + totalSamples + '条记录)\n';
                fishes.sort(([, a], [, b]) => b.length - a.length).forEach(([fishName, samples]) => {
                    const minS = Math.min(...samples.map(s => s.stamina));
                    const maxS = Math.max(...samples.map(s => s.stamina));
                    msg += '   [鱼种] ' + fishName + ': ' + samples.length + '条, 体力' + minS + '-' + maxS + '\n';
                });
            });
        ...
        function mainLoop() {
            ...
            if (currentState === STATE.FIGHT) {
                const val = findStaminaMax();
                if (val && val > maxStaminaThisFight) {
                    maxStaminaThisFight = val;
                    const name = predictFishNameByStamina(val);
                    if (name) currentFishName = name;
                }
                updateDisplay();
            } else if (currentState === STATE.REPLAY) {
                checkReplayAndLearn();
            } else if (currentState === STATE.IDLE && maxStaminaThisFight > 0) {
                setTimeout(tryCaptureWeight, 2000);
                maxStaminaThisFight = 0;
            }
            checkFishingLogAndLearn();
        }

        function tryCaptureWeight() {
            if (maxStaminaThisFight <= 0) return;
            let weight = null, fishName = null, length = null;
            const catchCards = document.querySelectorAll('.message-card--catch');
            if (catchCards.length > 0) {
                const latest = catchCards[catchCards.length - 1];
                const text = latest.textContent;
                const weightMatch = text.match(/([\d,.]+)\s*kg/);
                if (weightMatch) weight = parseFloat(weightMatch[1].replace(/,/g, ''));
                const nameMatch = text.match(/上鱼[：:]\s*(.+?)\s*·/);
                if (nameMatch) fishName = cleanFishName(nameMatch[1]);
                const lengthMatch = text.match(/\d+\.?\d*\s*kg\s*\/\s*([\d,.]+)\s*cm/);
                if (lengthMatch) length = parseFloat(lengthMatch[1].replace(/,/g, ''));
            }
            if (weight && weight > 0) {
                const envData = extractEnvironmentAndRegion() || {};
                addSample(maxStaminaThisFight, weight, fishName || currentFishName, { length: length, location: envData.location, environment: envData.environment, waterFlow: envData.waterFlow, humidity: envData.humidity });
            }
            maxStaminaThisFight = 0;
        }
        ...
        function startMonitoring() {
            ...
            const grid = document.querySelector('.fishing-metric-grid');
            if (grid) {
                const observer = new MutationObserver(() => { if (currentState === STATE.FIGHT) updateDisplay(); });
        ...
        function resetLearning() {
            globalData = []; speciesData = {}; regionalData = {}; speciesCoefCache = {};
            flushData(); recalcGlobalRegression();
            maxStaminaThisFight = 0; lastReplaySignature = ''; currentFishName = null;
            alert('所有学习数据已清除');
        }

        function registerModuleMenu() {
            const register = typeof GM_registerMenuCommand === 'function' ? GM_registerMenuCommand :
                             (typeof GM !== 'undefined' && typeof GM.registerMenuCommand === 'function') ? GM.registerMenuCommand : null;
            if (!register) return;
            register('重置体重学习数据', resetLearning);
            register('查看体重学习样本(双向互通)', showQueryDialog);
            register('从上鱼记录批量学习', batchLearnCatchHistory);
        }
        ...
        function init() {
            ...
            rebuildAllCoefCache();
            recalcGlobalRegression();
            extractEnvironmentAndRegion();
            console.log('[体重预测] 通用: ' + globalData.length + '条 | 鱼种: ' + Object.keys(speciesData).length + '种 | 区域: ' + Object.keys(regionalData).length + '个');
...
// 功能：主线切线计算 v3 - 简约版
// 注册名：mainLineCutCount
// ============================================================
function initMainLineCutCount() {
    (function() {
        'use strict';

        const REFRESH_INTERVAL = 15000;
        const CUT_DISTANCES = [50, 100, 150];
        let timer = null;

        function parseMainLineInfo(card) {
            var metaContainer = card.querySelector('.loadout-slot-meta');
            var statsContainer = card.querySelector('.loadout-slot-stats');

            var durability = null;
            if (metaContainer) {
                var duraSpan = Array.prototype.slice.call(metaContainer.querySelectorAll('span')).find(function(s) {
                    return s.textContent.includes('耐久') && /\d/.test(s.textContent);
                });
                if (duraSpan) {
                    var match = duraSpan.textContent.match(/耐久\s*([\d.]+)%/);
                    if (match) durability = parseFloat(match[1]);
                }
            }

            var lineLength = null;
            if (statsContainer) {
                var lengthSpan = Array.prototype.slice.call(statsContainer.querySelectorAll('span')).find(function(s) {
                    return s.textContent.includes('线长') && /\d/.test(s.textContent);
                });
                if (lengthSpan) {
                    var match = lengthSpan.textContent.match(/线长\s*([\d.]+)\s*m/);
                    if (match) lineLength = parseFloat(match[1]);
                }
            }

            return { durability: durability, lineLength: lineLength };
        }

        function findMainLineCard() {
            var slots = document.querySelectorAll('.loadout-slot');
            for (var i = 0; i < slots.length; i++) {
                var label = slots[i].querySelector('.loadout-slot-label');
                var hint = slots[i].querySelector('.loadout-slot-hint');
                if (label && label.textContent.trim() === '鱼线' && hint && hint.textContent.trim() === '必需') {
                    var nameEl = slots[i].querySelector('.loadout-slot-name');
                    if (nameEl && nameEl.textContent.includes('主线')) {
                        return slots[i];
                    }
                }
            }
        ...
        function calc(dist, durability, lineLength) {
            if (!durability || !lineLength || durability <= 0) return null;
            var cost = (dist / lineLength) * 100;
            if (cost <= 0) return null;
            return Math.floor(durability / cost);
        }

        function render(card, durability, lineLength) {
            var old = card.querySelector('.cut-count-display');
            if (old) old.remove();

            var display = document.createElement('div');
            display.className = 'cut-count-display';
            display.style.cssText = 'margin-top:4px;font-size:11px;color:#94a3b8;display:flex;gap:12px;';

            var html = '';
            for (var i = 0; i < CUT_DISTANCES.length; i++) {
                var dist = CUT_DISTANCES[i];
                var count = calc(dist, durability, lineLength);
                html += '<span>' + dist + 'm 可 切<b style="color:#38bdf8;"> ' + (count !== null ? count : '?') + ' </b>次</span>';
            }

            display.innerHTML = html;

            var stats = card.querySelector('.loadout-slot-stats');
            if (stats) {
                stats.parentNode.insertBefore(display, stats.nextSibling);
            } else {
        ...
        function update() {
            var card = findMainLineCard();
            if (!card) return;
            var info = parseMainLineInfo(card);
            if (info.durability === null || info.lineLength === null) return;
            render(card, info.durability, info.lineLength);
        }

        function init() {
            setTimeout(function() {
                update();
                timer = setInterval(update, REFRESH_INTERVAL);
            }, 2000);
        }
...
function initItemCardEnhance() {
    ...
    const INIT_MAP = {
        ...
        fishWeightPred: initFishWeightPred,
        ...
        mainLineCutCount: initMainLineCutCount,
