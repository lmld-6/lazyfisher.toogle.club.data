...
//消耗体力
// 注册名：initStaminaDrain
// 注册名：initStaminaDrain
// ============================================================
function initStaminaDrain() {
    (function() {
        'use strict';

        function calculateDrainRates(fishStamina) {
            const current = parseFloat(fishStamina) || 0;
            const base = current / 10;
            return {
                rate0: Math.max(1, Math.ceil(base * 1.00)),
                rate20: Math.max(1, Math.ceil(base * 0.80)),
                rate86: Math.max(1, Math.ceil(base * 0.14)),
                rate95: Math.max(1, Math.ceil(base * 0.05))
            };
        }

        function createDrainCard(rates, fishStaminaPercent) {
            const card = document.createElement('div');
            card.className = 'fishing-metric-card stamina-drain-card';
            card.setAttribute('data-drain-card', 'true');

            card.innerHTML = `
                <div class="fishing-metric-header">
                    <span class="fishing-metric-label">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-battery-warning icon-sm" aria-hidden="true">
                            <path d="M10 17h.01"></path><path d="M10 7v6"></path>
                    ...
                    <span class="fishing-metric-value">
                        <span class="drain-rate drain-rate-0" title="0% 减少体力压力">${rates.rate0}</span>
                        <span class="drain-separator">/</span>
                        <span class="drain-rate drain-rate-20" title="20% 减少体力压力">${rates.rate20}</span>
                        <span class="drain-separator">/</span>
                        <span class="drain-rate drain-rate-86" title="86% 减少体力压力">${rates.rate86}</span>
                        <span class="drain-separator">/</span>
                        <span class="drain-rate drain-rate-95" title="95% 减少体力压力(最大)">${rates.rate95}</span>
                    </span>
                </div>
                <div class="progress-bar fishing-metric-progress stamina-drain-bar">
                    <div class="progress-bar-fill progress-bar-fill--warning" style="width: ${fishStaminaPercent};"></div>
                </div>
            `;
        ...
        function injectStyles() {
            if (document.getElementById('stamina-drain-styles')) return;
            const style = document.createElement('style');
            style.id = 'stamina-drain-styles';
            style.textContent = `
                .stamina-drain-card {
                    border-left: 3px solid #f59e0b;
                    background: linear-gradient(135deg, #fffbeb 0%, #fef3c7 30%, #fff 100%);
                    order: 5 !important;
                }
                ...
                .stamina-drain-bar .progress-bar-fill--warning {
        ...
        function findFishStaminaCard(grid) {
            // 查找包含"鱼体力"标签的卡片
            const cards = grid.querySelectorAll(':scope > .fishing-metric-card');
            for (const card of cards) {
                const label = card.querySelector('.fishing-metric-label');
        ...
        function getFishStaminaData(fishCard) {
            const valueEl = fishCard.querySelector('.fishing-metric-value');
        ...
        function processFishingPanel() {
            const grid = document.querySelector('.fishing-metric-grid');
            if (!grid) return;

            const fishCard = findFishStaminaCard(grid);
            if (!fishCard) return;

            const data = getFishStaminaData(fishCard);
            if (!data) return;

            const pct = (data.current / data.max * 100).toFixed(2);
            const rates = calculateDrainRates(data.current);

            // 检查消耗卡是否已在鱼体力卡后面
            let drainCard = fishCard.nextElementSibling;
            let needsNewCard = true;

            if (drainCard && drainCard.classList.contains('stamina-drain-card')) {
                needsNewCard = false;
                // 更新现有卡片
                const rateEls = drainCard.querySelectorAll('.drain-rate');
                if (rateEls.length === 4) {
                    rateEls[0].textContent = rates.rate0;
                    rateEls[1].textContent = rates.rate20;
                    rateEls[2].textContent = rates.rate86;
                    rateEls[3].textContent = rates.rate95;
                }
                const bar = drainCard.querySelector('.progress-bar-fill');
                if (bar) bar.style.width = pct + '%';
            }

            if (needsNewCard) {
                // 移除旧的消耗卡（如果在别的位置）
                const oldCard = grid.querySelector('.stamina-drain-card');
                if (oldCard) oldCard.remove();

                drainCard = createDrainCard(rates, pct + '%');
                fishCard.insertAdjacentElement('afterend', drainCard);
            }
        }
        ...
        function init() {
            ...
            const observer = new MutationObserver(() => {
                clearTimeout(debounceTimer);
                debounceTimer = setTimeout(processFishingPanel, 100);
            });
